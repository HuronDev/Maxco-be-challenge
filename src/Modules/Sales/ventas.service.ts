...
export class VentasService {
  ...
  if (!ventaExistente) throw new NotFoundException(`Venta ${id} no encontrada`);
  ...
  const detallesConPrecio = await Promise.all(
    data.detalles.map(async (d) => {
      const producto = await this.prisma.producto.findUnique({ where: { id: d.productoId } });
      if (!producto) throw new NotFoundException(`Producto ${d.productoId} no encontrado`);
